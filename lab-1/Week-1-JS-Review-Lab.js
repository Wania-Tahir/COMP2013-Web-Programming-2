 ...
 */
//WRITE YOUR CODE BELOW
const text = `This house at ${listing1.address} is owned by ${listing1.currentOwner}`;
console.log(text);
/**
...
function realtorFees(listing) {
  ...
  let fee = price <= 450000 ? price * 0.025 : price * 0.02;
...
const listingAscendingly = listings.sort((listingA, listingB) => {
  const priceA = Number(listingA.price.replace("$", "").replace(",", ""));
  const priceB = Number(listingB.price.replace("$", "").replace(",", ""));
 ...
 */
//WRITE YOUR CODE BELOW
const newBuiltListings = listings.filter((listing) => listing.builtIn >= 1990);
